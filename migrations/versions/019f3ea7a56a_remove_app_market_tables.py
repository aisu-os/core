...
def downgrade() -> None:
    ...
    op.create_table(
        ...
        sa.UniqueConstraint(
            "app_id",
            "user_id",
            name=op.f("app_installs_app_id_user_id_key"),
            postgresql_include=[],
    ...
    op.create_table(
        ...
        sa.Column(
            "manifest", postgresql.JSONB(astext_type=sa.Text()), autoincrement=False, nullable=False
        ),
        ...
        sa.Column(
            "rating_avg", sa.NUMERIC(precision=3, scale=2), autoincrement=False, nullable=False
        ),
    ...
    op.create_table(
        ...
        sa.UniqueConstraint(
            "app_id",
            "version",
            name=op.f("app_versions_app_id_version_key"),
            postgresql_include=[],
            postgresql_nulls_not_distinct=False,
        ),
    )
    ...
    op.create_table(
        ...
        sa.UniqueConstraint(
            "app_id",
            "user_id",
            name=op.f("app_reviews_app_id_user_id_key"),
            postgresql_include=[],
            postgresql_nulls_not_distinct=False,
        ),
    )
    op.create_table(
        ...
        sa.Column(
            "granted_at",
            postgresql.TIMESTAMP(timezone=True),
            server_default=sa.text("'2026-02-15 22:01:26.416072+00'::timestamp with time zone"),
            autoincrement=False,
            nullable=False,
        ),
        sa.Column(
        ...
        sa.ForeignKeyConstraint(
            ["user_id"], ["users.id"], name=op.f("app_permissions_user_id_fkey")
        ),
        ...
        sa.UniqueConstraint(
            ...
            "user_id",
            "permission",
            name=op.f("app_permissions_app_id_user_id_permission_key"),
            postgresql_include=[],
            postgresql_nulls_not_distinct=False,
        ),
    )
